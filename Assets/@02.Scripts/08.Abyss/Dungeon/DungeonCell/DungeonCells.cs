...
public class DungeonCells
{
    ...
    void SwitchCorridorByNeighbour(int dir, Vector2Int cellPos, DungeonCells[,] cells, SODungeonTile dungeonTileSo)
    {
        ...
        if (dungeonOffset == null || dungeonOffset.mCellType == DungeonCellType.None)
        {
            ...
            SetTorch(dir, dungeonTileSo, dungeonOffset);
            return;
        }
    ...
    void SetTorch(int dir,SODungeonTile dungeonTileSo,DungeonCells dungeonOffset)
    {
        if (dungeonOffset.cellType == DungeonCellType.None || dungeonOffset.cellType == DungeonCellType.Room)
        {
            if ((dir == 1 || dir == 3) && cellPos.y % 3 == 0)
            {
                GameObject torch = GetStanderPrefab(dir, dungeonTileSo.torchPrefabs);
                torch.transform.SetParent(mDungeonStanderPrefab[dir].transform);
            }
            else if ((dir == 0 || dir == 2) && cellPos.x % 3 == 0)
            {
                GameObject torch = GetStanderPrefab(dir, dungeonTileSo.torchPrefabs);
                torch.transform.SetParent(mDungeonStanderPrefab[dir].transform);
            }
    ...
    public void SetCorridorCollider(Vector2Int fromCoord, Vector2Int toCoord,bool extraForCorner)
    {
        ...
        // GameObject ceilling = new GameObject();
        // ceilling.transform.SetParent(mCurrentTilePrefab.transform);
        // ceilling.transform.position = mCurrentTilePrefab.transform.position;
        // ceilling.transform.localScale = Vector3.one;
        // BoxCollider ceilingCol = ceilling.AddComponent<BoxCollider>();
        // ceilingCol.center += new Vector3(0.5f, 1.5f, 0.5f);
        ...
        if (Mathf.Abs(center.x) < Mathf.Abs(center.y))
        {
            ...
            // ceilingCol.center += new Vector3(0, 0, center.y * 0.5f);
            // ceilingCol.size = new Vector3(3, 1, center.y + Mathf.Sign(center.y));
        }
        else
        {
            ...
            // ceilingCol.center += new Vector3(center.x * 0.5f, 0, 0);
            // ceilingCol.size = new Vector3(center.x + Mathf.Sign(center.x), 1, 3);
        }
    }
