...
public class PopupPanelController : MonoBehaviour
{
    ...
    public virtual void Show()
    {
        ...
        mPanelRectTransform.localScale = Vector3.zero;
        
        //TODO: create UIManager
        UIPanelManager.Instance.PushPopup(this);
        
        // 배경은 등속으로 등장, 패널은 튕기듯이 등장
        mBackgroundCanvasGroup.DOFade(1, 0.3f).SetEase(Ease.Linear);
        mPanelRectTransform.DOScale(1, 0.3f).SetEase(Ease.OutBack);
    }
    ...
    {
        //TODO: create UIManager
        UIPanelManager.Instance.PopPopup(this);
        
        // 사라지기 전 초기화
        mBackgroundCanvasGroup.alpha = 1;
        mPanelRectTransform.localScale = Vector3.one;
        
        // 배경은 등속으로 퇴장, 패널은 빠르게 작아지다가 끝에서 살짝 당겨지듯 퇴장
        mBackgroundCanvasGroup.DOFade(0, 0.3f).SetEase(Ease.Linear);
        mPanelRectTransform.DOScale(0, 0.3f).SetEase(Ease.InBack).OnComplete(() =>
        {
